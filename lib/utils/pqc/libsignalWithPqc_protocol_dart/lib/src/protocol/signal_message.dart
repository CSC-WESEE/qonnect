...
// Write now the all the constants of CiphertextMessage are kept same.
class PqkemSignalMessage extends CiphertextMessage {
  PqkemSignalMessage(
      int messageVersion,
      Uint8List macKey,
      ECPublicKey senderRatchetKey,
  ...
  PqkemSignalMessage.fromSerialized(Uint8List serialized) {
    try {
      final messageParts = ByteUtil.split(
          serialized, 1, serialized.length - 1 - macLength, macLength);
      final version = messageParts[0].first;
