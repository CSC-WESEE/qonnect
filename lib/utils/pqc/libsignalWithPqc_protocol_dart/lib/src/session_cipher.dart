...
// ************************************************************** PQ Functions *****************************************************************************
class PqkemSessionCipher {
  PqkemSessionCipher(
      this._sessionStore,
      this._preKeyStore,
      this._pqkemPreKeyStore,
      SignedPreKeyStore signedPreKeyStore,
      PqkemSignedPreKeyStore pqkemSignedPreKeyStore,
      this._identityKeyStore,
      this._remoteAddress) {
    _sessionBuilder = PqSessionBuilder(
        _sessionStore,
        _preKeyStore,
        signedPreKeyStore,
        _identityKeyStore,
  ...
  SessionStore _sessionStore;
  IdentityKeyStore _identityKeyStore;
  late PqSessionBuilder _sessionBuilder;
  PreKeyStore _preKeyStore;
  ...
  Future<CiphertextMessage> encrypt(Uint8List paddedMessage) async {
    ...
    final previousCounter = sessionState.previousCounter;
    final sessionVersion = sessionState.getSessionVersion();
    final secretCipher = sessionState.secretCipher;

    // print('secret Cipher: $secretCipher');
    print(
        "Secret cipher from ratcheting_session........: ${secretCipher}");

    final ciphertextBody = getCiphertext(messageKeys, paddedMessage);
    print('Ciphertext: $ciphertextBody');
    CiphertextMessage ciphertextMessage = PqkemSignalMessage(
        sessionVersion,
        messageKeys.getMacKey(),
        senderEphemeral,
        chainKey.index,
    ...
    if (sessionState.hasUnacknowledgedPreKeyMessage()) {
      ...
      ciphertextMessage = PqkemPreKeySignalMessage.from(
        sessionVersion,
        localRegistrationId,
        items.getPreKeyId(),
        items.getSignedPreKeyId(),
        items.getBaseKey(),
        sessionState.getLocalIdentityKey(),
        ciphertextMessage as PqkemSignalMessage,
        secretCipher!,
        items.getPqkemSignedPreKeyId(),
        items.getPqkemSignedOneTimePreKeyId(),
      );
    }
  ...
  Future<Uint8List> decrypt(PqkemPreKeySignalMessage ciphertext) async =>
      decryptWithCallback(ciphertext, () {}());

  Future<Uint8List> decryptWithCallback(
      PqkemPreKeySignalMessage ciphertext, DecryptionCallback? callback) async {
    final sessionRecord = await _sessionStore.loadSession(_remoteAddress);
    final unsignedPreKeyId =
        await _sessionBuilder.process(sessionRecord, ciphertext);
    final plaintext = _decrypt(sessionRecord, ciphertext.getWhisperMessage());
  ...
  Future<Uint8List> decryptFromSignal(PqkemSignalMessage cipherText) async =>
      decryptFromSignalWithCallback(cipherText, () {}());

  Future<Uint8List> decryptFromSignalWithCallback(
      PqkemSignalMessage cipherText, DecryptionCallback? callback) async {
    if (!await _sessionStore.containsSession(_remoteAddress)) {
      throw NoSessionException('No session for: $_remoteAddress');
    }
  ...
  Uint8List _decrypt(
      SessionRecord sessionRecord, PqkemSignalMessage cipherText) {
    final previousStates = sessionRecord.previousSessionStates;
    final exceptions = <Exception>[];
  ...
  Uint8List _decryptFromState(
      SessionState sessionState, PqkemSignalMessage ciphertextMessage) {
    if (!sessionState.hasSenderChain()) {
      throw InvalidMessageException('Uninitialized session!');
    }
